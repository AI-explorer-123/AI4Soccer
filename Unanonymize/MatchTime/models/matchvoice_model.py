...
class matchvoice_model(nn.Module):
    def __init__(self,
                 ...
                 restricted_token_list_path="./Restricted_Token_List/llama3.pkl",
                 # Q-former part
                 max_frame_pos=128,
                 window=30,
                 num_query_tokens=32,
        ...
        self.llama_model = AutoModelForCausalLM.from_pretrained(
            llm_ckpt, torch_dtype=torch.bfloat16)
        self.llama_model.generation_config.pad_token_id = self.tokenizer.pad_token_id
        ...
        self.video_frame_position_embedding = self.video_frame_position_embedding.to(
            self.device)
        ...
        with open(restricted_token_list_path, 'rb') as file:
            self.token_ids_list = pkl.load(file)
        self.token_ids_list.append(128000)
        self.token_ids_list.append(128001)
        self.processor = RestrictTokenGenerationLogitsProcessor(
            allowed_token_id_list=self.token_ids_list)
    ...
    def forward(self, samples, validating=False):
        ...
        video_features = einops.rearrange(
            video_features, 'b t n f -> (b t) n f', b=batch_size, t=time_length)

        position_ids = torch.arange(
            time_length, dtype=torch.long, device=video_features.device)
        ...
        frame_atts = torch.ones(frame_hidden_state.size(
        )[:-1], dtype=torch.long).to(frame_hidden_state)
        video_query_tokens = self.video_query_tokens.expand(
            frame_hidden_state.shape[0], -1, -1).to(frame_hidden_state.device)
        ...
        if validating:
            ...
            ground_truth_comment = [sublist[3] for sublist in samples["caption_info"]]
            anonymous_comment = [sublist[6] for sublist in samples["caption_info"]]
            file_paths = [sublist[7] for sublist in samples["caption_info"]]
            return temp_res_text, ground_truth_comment, anonymous_comment, file_paths

        if hasattr(self.llama_model, "hf_device_map"):
            # 找到模型的某个组件的设备作为参考
            llama_device = next(self.llama_model.parameters()).device
        else:
            llama_device = self.llama_model.device if hasattr(
                self.llama_model, "device") else self.device
        
        visual_label = torch.full(
            (batch_size, self.num_video_query_token), -100, dtype=targets.dtype)
        concat_targets = torch.cat(
            (visual_label, targets), dim=1).to(llama_device)
        temp_input_ids = inputs_ids.clone().to(self.device)
        targets_embeds = self.llama_model.model.embed_tokens(temp_input_ids)
        embedding_cat = torch.cat((inputs_llama, targets_embeds), dim=1).to(llama_device)
        mask_prefix = torch.ones(
            batch_size, self.num_video_query_token, dtype=atts_llama.dtype)
        mask = torch.concat((mask_prefix, atts_llama),
                            dim=1).to(llama_device)
    ...
    def generate_text(self, inputs_llama):
        start_embeds = self.llama_model.model.embed_tokens(
            torch.tensor([128000]).to(self.device))
        inputs_llama_with_s = torch.cat([inputs_llama, start_embeds.expand(
            inputs_llama.size(0), -1, -1)], dim=1).to(dtype=torch.bfloat16)
