...
class QwenVLCommentaryDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if not self.train:
            extra_info = {
                "comments_text_anonymized": video_info.get("comments_text_anonymized", ""),
                "original_video_path": video_info["original_video_path"]
            }

        return chat, extra_info if not self.train else chat
    ...
    def collate_fn(self, examples):
        ...
        if self.train:
            ...
            chats = [example[0] for example in examples]
            extra_infos = [example[1] for example in examples]

        # 1. 用processor.apply_chat_template生成文本
        if self.train:
            texts = [self.processor.apply_chat_template(
                example, tokenize=False) for example in chats]
        else:
            texts = [self.processor.apply_chat_template(
                example[:2], tokenize=False, add_generation_prompt=True) for example in chats]
        ...
        if not self.train:
            caption_text = [example[-1]["content"][0]["text"]
                            for example in chats]
            return batch, caption_text, extra_infos
